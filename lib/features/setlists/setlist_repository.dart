...
class SetlistRepository {
  ...
  Future<void> deleteSongFromSetlist({
    ...
    try {
      ...
      debugPrint(
        '[SetlistRepository] Removed song $songId from setlist $setlistId',
      );
    } catch (e) {
  ...
  Future<void> deleteSongFromCatalog({
    ...
    try {
      ...
      if (setlistIds.isNotEmpty) {
        ...
        debugPrint(
          '[SetlistRepository] Removed song $songId from ${setlistIds.length} setlists',
        );
      }
      ...
      debugPrint('[SetlistRepository] Deleted song $songId from band $bandId');
    } catch (e) {
  ...
  ///
  /// IMPORTANT: Cannot duplicate the Catalog setlist.
  Future<String> duplicateSetlist({
    ...
    try {
      ...
      // Step 2: Prevent duplication of Catalog
      final sourceName = sourceSetlist['name'] as String;
      if (sourceName.toLowerCase() == 'catalog') {
        throw SetlistQueryError(
          code: 'CATALOG_PROTECTED',
          message: 'Cannot duplicate the Catalog setlist',
          reason: 'catalog_protected',
        );
      }
  ...
  Future<String> _ensureCatalogClientSide(String bandId) async {
    ...
    // Create new Catalog (handle case where is_catalog column may not exist)
    try {
      try {
        ...
        if (e.code == '42703') {
          // is_catalog column doesn't exist, create without it
          final newCatalog = await supabase
              .from('setlists')
              .insert({
                'band_id': bandId,
          ...
          if (kDebugMode) {
            debugPrint(
              '[SetlistRepository] Created Catalog via client-side (no is_catalog): ${newCatalog['id']}',
            );
          }
          return newCatalog['id'] as String;
        }
      ...
      debugPrint('[SetlistRepository] Error creating Catalog client-side: $e');
      throw SetlistQueryError(
        code: 'CREATE_FAILED',
  ...
  Future<String?> _createOrFindSong({
    ...
    debugPrint(
      '[SetlistRepository] _createOrFindSong: title=$title, artist=$artist, bpm=$bpm, tuning=$tuning',
    );
    try {
      ...
      if ((existing as List).isNotEmpty) {
        ...
        if (kDebugMode) {
          debugPrint(
            '[SetlistRepository] Found existing song: $title by $artist -> $existingId',
          );
        }
        ...
        if (bpm != null && existing[0]['bpm'] == null) {
          updates['bpm'] = bpm;
        }
      ...
      if (kDebugMode) {
        debugPrint(
          '[SetlistRepository] Created new song: $title by $artist -> $newId',
        );
      }
      return newId;
    } on PostgrestException catch (e) {
