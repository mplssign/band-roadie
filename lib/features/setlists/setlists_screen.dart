...
class SetlistsNotifier extends Notifier<SetlistsState> {
  ...
  Future<void> loadSetlists() async {
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('[SetlistsScreen] Loaded ${setlists.length} setlists');
      }
      ...
      if (kDebugMode) {
        debugPrint('[SetlistsScreen] Error loading setlists: $e');
      }
...
class _SetlistsScreenState extends ConsumerState<SetlistsScreen>
  ...
  void _navigateToCreateSetlist() {
    Navigator.of(
      context,
    ).push(MaterialPageRoute(builder: (context) => const NewSetlistScreen()));
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (!setlistsState.isLoading && setlistsState.setlists.isNotEmpty) {
      final itemCount = setlistsState.setlists.length + 1; // +1 for header
    ...
    final showError = setlistsState.error != null;
    final showEmpty =
        !showLoading && !showError && setlistsState.setlists.isEmpty;
    ...
    return DrawerOverlay(
      ...
      child: BandSwitcherOverlay(
        ...
        onCreateBand: () {
          ...
          Navigator.of(
            context,
          ).push(MaterialPageRoute(builder: (_) => const CreateBandScreen()));
        },
        onEditBand: () {
          ...
          if (activeBand != null) {
            ...
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => EditBandScreen(band: activeBand),
              ),
            );
          }
        },
