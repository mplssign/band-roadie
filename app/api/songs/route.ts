    ...
    for (const searchUrl of searches) {
      try {
        ...
        const filteredResults = data.results?.filter(track => {
          ...
          const normalizedTitle = track.trackName.toLowerCase().replace(/[^\w\s]/g, '').replace(/\s+/g, ' ').trim();
          const normalizedArtist = track.artistName.toLowerCase().replace(/[^\w\s]/g, '').replace(/\s+/g, ' ').trim();
...
const BILLBOARD_CHART_HITS: Record<string, { artist: string; peak_position: number; year: number }[]> = {
  ...
  'bullet with butterfly wings': [{ artist: 'The Smashing Pumpkins', peak_position: 22, year: 1995 }],
...
async function fetchSongsterrTuning(title: string, artist: string): Promise<string[]> {
  try {
    ...
    const bestMatch = data.find(track => {
    ...
    if (trackDetailResponse.ok) {
      ...
      const guitarTunings = trackDetail.tunings?.filter(t => 
...
const POPULAR_ARTISTS = new Set([
  'pearl jam', 'nirvana', 'soundgarden', 'alice in chains', 'stone temple pilots',
  'metallica', 'megadeth', 'slayer', 'anthrax', 'iron maiden',
  'led zeppelin', 'the beatles', 'pink floyd', 'queen', 'the rolling stones',
  'ac/dc', 'guns n\' roses', 'guns n roses', 'gnr', 'aerosmith', 'van halen', 'black sabbath',
  'def leppard', 'bon jovi', 'journey', 'foreigner', 'boston',
  'radiohead', 'foo fighters', 'red hot chili peppers', 'the killers', 'green day',
  'linkin park', 'system of a down', 'tool', 'rage against the machine', 'muse',
  // Legendary solo artists
  'john lennon', 'paul mccartney', 'bob dylan', 'david bowie', 'elvis presley',
  'johnny cash', 'bruce springsteen', 'prince', 'michael jackson', 'stevie wonder',
  'neil young', 'eric clapton', 'jimi hendrix', 'jim morrison', 'the doors',
  'leonard cohen', 'joni mitchell', 'frank sinatra', 'ray charles', 'sam cooke'
]);
...
function sortiTunesResults(results: iTunesTrack[], query: string): iTunesTrack[] {
  ...
  const scoredResults = results.map(track => {
    const title = (track.trackName || '').toLowerCase().replace(/[^\w\s]/g, '').replace(/\s+/g, ' ').trim();
    const artist = (track.artistName || '').toLowerCase().replace(/[^\w\s]/g, '').replace(/\s+/g, ' ').trim();
    ...
    if (chartData) {
      const matchingChart = chartData.find(chart => {
        const chartArtist = chart.artist.toLowerCase().replace(/[^\w\s]/g, '').replace(/\s+/g, ' ').trim();
        return chartArtist === artist || originalArtist.includes(chartArtist) || chartArtist.includes(artist);
      });
    ...
    if (title === queryLower || originalTitle === queryLower) {
      ...
      for (const queryWord of queryWords) {
        if (titleWords.some(titleWord => titleWord === queryWord) || 
          ...
          exactWordMatches++;
        } else if (titleWords.some(titleWord => titleWord.includes(queryWord)) ||
    ...
    if (isPopularArtist) {
      ...
      const legendaryBands = ['guns n roses', 'guns n\' roses', 'gnr', 'led zeppelin', 
                             'queen', 'the beatles', 'metallica', 'ac dc', 'acdc',
                             'pink floyd', 'the rolling stones', 'rolling stones',
                             'nirvana', 'pearl jam', 'black sabbath',
                             // Legendary solo artists
                             'john lennon', 'paul mccartney', 'bob dylan', 'david bowie',
                             'elvis presley', 'johnny cash', 'bruce springsteen', 'prince',
                             'michael jackson', 'stevie wonder', 'neil young', 'eric clapton',
                             'jimi hendrix', 'jim morrison', 'the doors', 'leonard cohen',
                             'joni mitchell', 'frank sinatra', 'ray charles', 'sam cooke'];
      const isLegendaryBand = legendaryBands.includes(artist) || legendaryBands.includes(originalArtist);
      if (isLegendaryBand) {
        score += 500; // Additional legendary band bonus
        
        // If it's a good title match AND legendary band, give massive boost
        if (score >= 1000) { // Has decent title match
    ...
    if (score === 0) { // Only if no title match found
    ...
    if (score > 0) {
      ...
      const isLikelyOriginalArtist = !artist.includes('tribute') && 
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const existingTitleArtistPairs = new Set(
      updatedExistingSongs.map(song => `${song.title.toLowerCase()}_${song.artist.toLowerCase()}`)
    );

    const newSongs = normalizedResults.filter(song => 
